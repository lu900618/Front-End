...
- 2.灵活的渐进式框架
    ...
    *单页面就需要路由。项目比较复杂庞大，就需要引进vuex，来实现集中管理状态，整个项目构建完成之后还需要构建工具优化build我们的系统，来提升效率，形成一个完整的项目，这就是一个渐进式的过程，也就是说构建一个项目，从简单到复杂，随着业务度的提升，不断的加入和引用其他的插件，比如如果需要和后台交互，就需要引入一个vue-resource插件，做单页面应用，就可以引用一个vue-router，当我们的项目足够复杂，就引用 vuex ，最后通过built工具来构建我们的系统。也就是说做项目得过程中缺什么，补什么，这就是一个渐进式的过程。*
- 3.高效
- 4.虚拟DOM
